...
### 源
此参数枚举了可用的输出容器。TWFP_POCKET1到TWFP_POCKET16依次识别为从上到下，从左到右。
...
### 源
如果CAP_FEEDERENABLED为TRUE，并且CAP_FEEDERPREP也为TRUE，那么扫描仪将会采取一切必要措施来加快进纸速度。  
如果不支持，返回TWRC_FAILURE / TWCC_CAPUNSUPPORTED。
...
### 描述
如果为TRUE，那么源将会推出当前页，并从自动进纸器加载下一页。  
如果CAP_AUTOFEED为TRUE，那么源的动作和上面保持一致，并且CAP_AUTOFEED保持TRUE。  
CAP_FEEDERENABLED必须为TRUE方可使用此参数。  
此参数在状态5和状态6之间使用必须被当做拓展参数。

### 应用程序
使用MSG_SET将此参数设置为TRUE，以便于加载下一页。  
此参数在状态5和状态6之间用于控制源进纸。  
此参数同样可以在CAP_AUTOFEED为TRUE的时候使用，推出当前所有页，并加载下一页。

### 源
如果CAP_FEEDERENABLED为FALSE，返回TWRC_FAILURE / TWCC_CAPSEQERROR。  
如果支持，那么源会将当前所有页推出到输出区域，并中止所有挂起的转换页。
...
### 描述
如果为TRUE，无论图形界面是否被激活，源都会显示进度指示器。如果为FALSE，进度指示器则被禁止。  

### 应用程序
如果应用程序使用了TW_USERINTERFACE参数，并且ShowUI = FALSE，那么使用此参数同样可以禁止指示器显示。
